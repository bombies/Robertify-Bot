...
class GeniusSongSearch(private val gla: GeniusAPI, query: String, page: Int = 1) {
    ...
    private fun request(uri: URI) {
        try {
            val request = Request.Builder()
                .url(uri.toURL())
                .get()
                .addHeader("User-Agent", BotConstants.USER_AGENT)
    ...
    class Hit(private val gla: GeniusAPI, jRoot: JSONObject) {
        val id: Long
        val title: String
        ...
        fun fetchLyrics(): String? =
            GeniusLyricsParser(gla).get(id.toString() + "")
