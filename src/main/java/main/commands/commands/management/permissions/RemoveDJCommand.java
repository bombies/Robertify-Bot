...
public class RemoveDJCommand implements ICommand {
    @Override
    public void handle(CommandContext ctx) throws ScriptException {
        ...
        try {
            permissionsConfig.removeRoleFromPermission(guild.getId(), id, Permission.ROBERTIFY_DJ);
            eb = EmbedUtils.embedMessage("Removed " + role.getAsMention() + " as a DJ!");
            msg.replyEmbeds(eb.build()).queue();
        } catch (IllegalAccessException e) {
            eb = EmbedUtils.embedMessage("This role never was a DJ in the first place");
            msg.replyEmbeds(eb.build()).queue();
        } catch (Exception e) {
            e.printStackTrace();
