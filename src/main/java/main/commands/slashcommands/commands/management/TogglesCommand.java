...
public class TogglesCommand extends AbstractSlashCommand {
    private final Logger logger = LoggerFactory.getLogger(TogglesCommand.class);

    private void displayDJToggles(SlashCommandInteractionEvent event, SlashCommandManager commandManager, TogglesConfig config) {
        final var musicCmds = commandManager.getMusicCommands();
        final var description = new ArrayList<String>();
        ...
        description.add("""
                ...
                %s->%s
                ```""".formatted(
                        localeManager.getMessage(RobertifyLocaleMessage.TogglesMessages.DJ_TOGGLES_EMBED_STATUS_FIELD),
                localeManager.getMessage(RobertifyLocaleMessage.TogglesMessages.DJ_TOGGLES_EMBED_COMMAND_FIELD))
        );

        musicCmds.sort(Comparator.comparing(AbstractSlashCommand::getName));
        for (AbstractSlashCommand toggle : musicCmds)
            description.add("%s        %s".formatted(config.getDJToggle(toggle) ? RobertifyEmoji.CHECK_EMOJI.toString() : RobertifyEmoji.QUIT_EMOJI.toString(), toggle.getName()));

        PaginationHandler.paginateMessage(description, 25, event);
    }
    ...
    public void onSlashCommandInteraction(@NotNull SlashCommandInteractionEvent event) {
        ...
        switch (path[1]) {
            ...
            case "dj" -> {
                switch (path[2]) {
                    case "list" -> {
                        displayDJToggles(event, SlashCommandManager.getInstance(), config);
                        return;
                    }
                    case "switch" -> {
